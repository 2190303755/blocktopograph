...
class LongNode(
    ...
    @Composable
    override fun icon() = painterResource(R.drawable.ic_tag_long)
    ...
    override fun Editor(editor: NBTEditorModel) {
        ...
        OutlinedTextField(
            ...
            supportingText = {
                // TODO: i18n
                Text(if (isError) "Invalid long" else "", Modifier.clearAndSetSemantics {})
            },
    ...
    inner class Assign(val neo: Long) : Operation {
        val old: Long = this@LongNode.value
        override fun redo(editor: NBTEditorModel) {
            this@LongNode.value = this.neo
        }

        override fun undo(editor: NBTEditorModel) {
            this@LongNode.value = this.old
        }
    }
}